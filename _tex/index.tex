...
\def\labelenumi{\arabic{enumi}.}
\item
  Model degrades as lag time increases.
  ...
  \end{itemize}
\item
  However, performance is still clinically meaningful even with a two
  week lag.

  \begin{itemize}
  \tightlist
  \item
    .85 auROC is still excellent and the benefit of advanced notice may
    outweigh the drop in performance.
  \end{itemize}
\item
  ...
  \tightlist
  \item
    Acknowledge low N in minority groups for income and race/ethnicity
    (causing wide CIs?)
  \end{itemize}
\item
  Discussion of Shapley values.

  \begin{itemize}
  \tightlist
  \item
    local shapley plots suggest what someone needs is different and
    different moments (within and between differences).
  \item
    Difference between slow changing states (e.g., income) and more
    dynamic states (e.g., craving).
  \end{itemize}
\item
  Future directions
  ...
  \tightlist
  \item
    Explore methods for improving model performance for groups
    underrepresented in the data, such as upsampling and adjusting
    penalties for minority groups (fairness paper).
  \item
    Recruit a more representative sample to improve diversity of
    training data (RISK2 grant).
  \item
    Build personalized models that inherently capture time series data
    and how one's risk evolves over time (state-space models).
  \end{itemize}
\end{enumerate}
