...
Odd truncated distribution in split 3 - *May explain weird peaks seeing in histogram of all probabilities*
...
### Possible Solutions from Chatgpt

1. Try the "ab" parameterization instead of "abm"
In R’s {betacal} package, beta calibration supports two variants:
- "abm" (more flexible but harder to fit) *Could explore what harder to fit means*
- "ab" (simpler, more stable)

*I wasn't able to change this parameter inside `beta_calibration()` function so this is a work around.* 
```{r}
# beta calibration
library(betacal)
beta <- predict(model_cal, feat_cal_out,
...
cal_model <- betacal::beta_calibration(beta$.pred_yes, feat_cal_out$y, parameters = "ab")

class(cal_model) <- "beta_cal"

# I had to manually define the predict() method for "beta_cal" objects
predict.beta_cal <- function(object, x, ...) {
  lp <- log(x)
  l1p <- log(1 - x)
  pred_lp <- object$map[1] * lp + object$map[2] * l1p
  1 / (1 + exp(-pred_lp))
}

beta_d_out <- predict(cal_model, preds_prob$.pred_yes)
```
...
```{r}
preds_3_old <- fit_predict_eval(3, splits, configs_best)
```
