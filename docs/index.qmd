...
3. Figure with auroc and confidence interval by demographic group and lag 
...
ci_baseline <- read_csv(file.path(path_models_lag, "ci_baseline.csv"), 
                        col_types = cols())

ci_lag <- read_csv(file.path(path_models_lag, "ci_lag.csv"), col_types = cols())

pp_dem <- read_csv(file.path(path_models_lag, "pp_dem_all.csv"), col_types = cols())

pp_dem_contrast <- read_csv(file.path(path_models_lag, "pp_dem_contrast_all.csv"), col_types = cols())

screen <- read_csv(file.path(path_processed, "dem_tibble.csv"),
                   col_types = cols())

lapses_per_subid <- read_csv(file.path(path_processed, "lapse_tibble.csv"),
                             col_types = cols())
```
...
Precision = approaches that are algorithmic, quantitative, and empirically derived and lead to data-informed and data-driven psychological therapies (treatment matching at baseline and ongoig dynamic prediction systems that are automatically updated to a patient's needs)

Personalized = umbrella term that describes any effort by the clinician to select, adapt, or adjust treatment to an individual with the goal of improving outcomes, including approaches that originate from and are driven by a holistic and person-centered approach-->


Precision medicine has been a goal in healthcare for over half a century [@derubeisHistoryCurrentStatus2019]. Traditionally, precision medicine seeks to answer *how* do we best treat a specific individual (e.g., which medication will work best for an individual given their unique background, preferences, and medical history). 

Today, this approach is also applied to chronic mental health conditions (i.e., precision mental health), like depression, substance use disorders, and suicide [<!--add citations-->]. However, mental health conditions are complex fluctuating processes. Unlike medical conditions that have clear biological precursors, mental health conditions are products of lots of psychosocial factors. These factors differ between individuals and within an individual over time. Additionally, there are a wide array of psychosocial supports available for clinicians to select from. Thus, precision mental health must consider both the *when* and the *how*.

There has been long-standing interest in precision mental health and substance use disorders. In an early example, The Project MATCH research group attempted to match individuals with alcohol use disorder to their optimal treatment based on baseline measures of individual differences [@projectmatchresearchgroupMatchingAlcoholismTreatments1997]. The study failed to find any differences in treatment success. This can likely be attributed to low dimensional analyses that failed to capture to the complex and heterogeneous nature of substance use disorders and their use of static distal features to predict a non-linear time-varying course of recovery, lapse, and relapse [@witkiewitzModelingComplexityPosttreatment2007]. 

Take the example of lapses, discrete instances of goal inconsistent substance use. Lapses are a clinically important target for substance use treatment. They are often an early warning sign of relapse, maladaptive responses to lapses can undermine recovery, and for some substances a single lapse can result in an overdose and/or death. It would be unreasonable to expect that we could predict a lapse with any temporal precision using only features that become more distal as time progresses. Lapse prediction requires dense, long-term monitoring of symptoms and related states proximal to the outcome.

Recent advances in both personal sensing and machine learning may address both of these barriers to successful precision mental health. Personal sensing allows for frequent, longitudinal measurement of changes in proximal risk (e.g., lapse) with high temporal precision. Machine learning allows for high dimensional inputs. Moreover, tools from machine learning can be applied to models to understand which factors are specific to a specific individual at a specific moment in time. 
...
<!--Add note about how all possible configurations were trained on inner loop but only best XGBoost considered for outer loop? Feels weird to mention here before cv section. Maybe this goes in results? Or as footnote?-->
...
There were approximately equal numbers of men (N=`r nrow(subset(screen, dem_2 == "Male"))`; `r sprintf("%1.1f", nrow(subset(screen, dem_2 == "Male"))/nrow(screen)*100)`%) and women (N=`r nrow(subset(screen, dem_2 == "Female"))`; `r sprintf("%1.1f", nrow(subset(screen, dem_2 == "Female"))/nrow(screen)*100)`%) who ranged in age from `r screen |> pull(dem_1) |> min()` - `r screen |> pull(dem_1) |> max()` years old.  The sample was majority White (N=`r nrow(subset(screen, dem_3 == "White/Caucasian"))`; `r  sprintf("%1.1f", nrow(subset(screen, dem_3 == "White/Caucasian"))/nrow(screen)*100)`%) and non-Hispanic (N=`r nrow(subset(screen, dem_4 == "No, I am not of Hispanic, Latino, or Spanish origin"))`; `r  sprintf("%1.1f", nrow(subset(screen, dem_4 == "No, I am not of Hispanic, Latino, or Spanish origin"))/nrow(screen)*100)`%).  Participants self-reported a median of `r sprintf("%1.1f",screen |> pull(dsm5_total) |> median())` DSM-5 symptoms of AUD (mean=`r sprintf("%1.1f",screen |> pull(dsm5_total) |> mean())`; SD=`r sprintf("%1.1f",screen |> pull(dsm5_total) |> sd())`; range=`r sprintf("%1.1f",screen |> pull(dsm5_total) |> min())`-`r sprintf("%1.1f",screen |> pull(dsm5_total) |> max())`). Most participants (N=`r nrow(subset(lapses_per_subid, n >0))`; `r sprintf("%1.1f", nrow(subset(lapses_per_subid, n >0))/nrow(lapses_per_subid)*100)`%) reported one or more lapses during participation.  The median number of lapses per participant while on-study was `r sprintf("%1.1f",lapses_per_subid |> pull(n) |> median())` (mean=`r sprintf("%1.1f",lapses_per_subid |> pull(n) |> mean())`; SD = `r sprintf("%1.1f",lapses_per_subid |> pull(n) |> sd())`; range=`r sprintf("%1.1f",lapses_per_subid |> pull(n) |> min())`-`r sprintf("%1.1f",lapses_per_subid |> pull(n) |> max())`).  [@tbl-demohtml]{.content-visible when-format="html"}[@tbl-demopdf]{.content-visible unless-format="html"} provides more detail on demographic and lapse characteristics of the sample.
...
[Table 2: Demographic and Lapse Characteristics (Continued)]{.center}
...
@fig-fairness shows the difference in performance of each model by demographic group. All group comparisons for race, sex, and income were significant (probability > .95) across models. On average there was a median increase in auROC for white participants compared to participants who were not white of `r sprintf("%1.3f", pp_dem_contrast |> filter(contrast == "non-hispanic white vs not white") |> pull(median)) |> median()` (range `r sprintf("%1.3f", pp_dem_contrast |> filter(contrast == "non-hispanic white vs not white") |> pull(median)) |> min()`-`r sprintf("%1.3f", pp_dem_contrast |> filter(contrast == "non-hispanic white vs not white") |> pull(median)) |> max()`). On average there was a median increase in auROC for male participants compared to female participants of `r sprintf("%1.3f", pp_dem_contrast |> filter(contrast == "male vs female") |> pull(median)) |> median()` (range `r sprintf("%1.3f", pp_dem_contrast |> filter(contrast == "male vs female") |> pull(median)) |> min()`-`r sprintf("%1.3f", pp_dem_contrast |> filter(contrast == "male vs female") |> pull(median)) |> max()`). On average there was a median increase in auROC for participants above the federal poverty line compared to participants below the federal poverty line of `r sprintf("%1.3f", pp_dem_contrast |> filter(contrast == "above poverty vs below poverty") |> pull(median)) |> median()` (range `r sprintf("%1.3f", pp_dem_contrast |> filter(contrast == "above poverty vs below poverty") |> pull(median)) |> min()`-`r sprintf("%1.3f", pp_dem_contrast |> filter(contrast == "above poverty vs below poverty") |> pull(median)) |> max()`).  


{{< embed notebooks/mak_figures.qmd#fig-fairness >}}
