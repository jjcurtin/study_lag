...
Alternatively, these images can be saved in a repository that is
...
You can also reference other sections in the paper (e.g., Methods are
described in Section~\ref{sec-methods})

You can reference them elsewhere using the @ symbol. Here is a reference
to Figure~\ref{fig-1}

We can also reference this type of figure elsewhere if we want
(Figure~\ref{fig-map}).
...
We can also print results (HOW???) \texttt{r\ summary(m)}
