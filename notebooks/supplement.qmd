...
### Figure S1: Calibration Plots for all Models
...
### Figure S2: Global Shapley Values as a Function of Lag Time
...
### Table S1: Alternative performance metrics
...
get_metrics <- function(preds) {
  ...
  preds |>
    ...
    summary() |>
    select(-.estimator) |>
    filter(.metric %in% c("sens", "spec", "accuracy", "bal_accuracy")) |> 
...
```{r}
#| label: tbl-metrics
#| tbl-cap: "Sensitivity, specificity, accuracy, and balanced accuracy performance estimates for each model at a specific decision threshold identified by Youden’s index."
#| 
metrics |> 
  ...
  select(model, sensitivity = sens, specificity = spec, 
         accuracy, `balanced accuracy` = bal_accuracy) |> 
