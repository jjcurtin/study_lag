...
preds_all_iso <- preds_0 |> 
  mutate(prob_iso = if_else(prob_iso == 0, .000001, prob_iso),
         bins = cut(prob_iso, breaks = seq(0, 1, bin_width)), 
         lapse = if_else(label == "Lapse", 1, 0)) |> 
  mutate(model = "No lag") |> 
  bind_rows(preds_24 |> 
              mutate(prob_iso = if_else(prob_iso == 0, .000001, prob_iso),
...
plot_cal <- preds_all |> 
  ...
  geom_rug(data = preds_all_raw, 
           aes(x = prob_raw, group = model), 
           sides = "b", 
           alpha = 0.1,
           color = "grey",
           inherit.aes = FALSE) +
  ...
  scale_y_continuous(breaks = seq(0, 1, bin_width), limits = c(0, 1)) +
  scale_color_manual(values = c("#21908C", "#5DC863", "#FDE725", "#440154")) +
