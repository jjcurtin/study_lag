...
panel_shap_global <- global_all |>
   mutate(model = factor(model, levels = c("336 lag", "168 lag", "72 lag", "24 lag", "0 lag" ),
                         labels = c("two weeks", "1 week", "3 days", "1 day", "No lag" ))) |> 
  ggplot() +
...
baseline_sex <- global_feat_0 |> 
  ...
  ggplot() +
  geom_point(aes(x = variable_grp, y = mean_value, group = group, 
                 color = group), size = 2, 
             position = position_dodge(width = .75)) +
  geom_segment(aes(x = as.numeric(variable_grp) + 
                     (as.numeric(factor(group)) - 1.5) * 0.4, 
      xend = as.numeric(variable_grp) + (as.numeric(factor(group)) - 1.5) * 
        0.4,  y = 0, yend = mean_value, group = group, color = group), 
               size = .5) +
  ...
  scale_color_manual(values = c("#cb6bce", "#240e31")) +
  coord_flip()
```
