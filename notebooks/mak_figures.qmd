...
fig_2a <- test_metrics_all_pp_perf |>
  ...
  scale_x_continuous("Model Lag", breaks = c(0, 24, 72, 168, 336), 
                     labels = c("0", "1 day", "3 days", "1 week",
                                "2 weeks")) +
  theme_classic() +
...
pp_dem_a <- pp_dem |> 
  ...
  facet_wrap(~ contrast) +
  scale_y_continuous("auROC", limits = c(.50, 1.0)) +
  scale_x_discrete(NULL, labels = c("0", "2 weeks")) +
  theme_classic() +
...
pp_dem_b <- pp_dem |> 
  ...
  scale_x_discrete("Model Lag", labels = c("0", "2 weeks")) +
  theme_classic() +
  theme(panel.border = element_rect(colour = "black", fill = NA, linewidth = 1)) +
  scale_color_manual(values = c("#240e31", "#cb6bce")) +
  geom_hline(yintercept = 0.5, linetype = "dashed", color = "black") +
...
panel_shap_local_0 <- shap_feat_0 |>
  ggplot(mapping = aes(x = variable_grp, y = shapley_value, 
                       color = feature_value)) +
  ggforce::geom_sina(method = "counts", maxwidth = .7, alpha = .7) +
  geom_hline(yintercept = 0) +
  scale_y_continuous(limits = c(-2, 5), breaks = seq(-2, 5)) +
  ylab("Shapley Value (No Lag)") +
  xlab(NULL) +
  scale_color_gradientn(colors = c("#52144d", "#f0c1ec"),
                        breaks = c(.1, .9),
                        labels = c("low", "high")) +
  labs(color = NULL) +
...
panel_shap_local_336 <- shap_feat_336 |>
  ...
  xlab(NULL) +
  scale_color_gradientn(colors = c("#295157", "#cbe3e7"),
                        breaks = c(.1, .9),
                        labels = c("low", "high")) +
  labs(color = NULL) +
  theme(legend.position = "right",
