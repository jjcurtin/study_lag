...
<!-- 8/3/24 - At 4907 words not including 150 word abstract or discussion.-->
...
1. Model degrades as lag time increases.
    
    - Features become less proximal to our outcome
  
2. However, performance is still clinically meaningful even with a two week lag.

    - .85 auROC is still excellent and the benefit of advanced notice may outweigh the drop in performance. 

3. All models were worse for women and people were not white and below poverty line. 
    
    - Acknowledge low N in minority groups for income and race/ethnicity (causing wide CIs?)
    
4. Discussion of Shapley values.

    - local shapley plots suggest what someone needs is different and different moments (within and between differences). 
    - Difference between slow changing states (e.g., income) and more dynamic states (e.g., craving).

5. Future directions

    - Explore methods for improving model performance for groups underrepresented in the data, such as upsampling and adjusting penalties for minority groups (fairness paper).
    - Recruit a more representative sample to improve diversity of training data (RISK2 grant).
    - Build personalized models that inherently capture time series data and how one's risk evolves over time (state-space models). 
