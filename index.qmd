...
<!--KW: Refer to calibration plots in supplement after those are created-->
...
Our models performed exceptionally well with median posterior probabilities for auROCs of .84 - .90 <!--not fair to lump short lags and long lags together.  Either focus only on the most important 2week lag or unpack performance separately for the short lag vs. long lag models-->. This suggests we can achieve clinically meaningful performance up to two weeks out. Our rigorous resampling methods (grouped, nested, k-fold cross-validation) make us confident that these are valid estimates of how our models would perform with new individuals.  

Nevertheless, model performance did decrease as models predicted further into the future. This is unsurprising given what we know about prediction and substance use. Many important relapse risk factors are fluctuating processes that can change day-by-day, if not more frequently. As lag time increases, features become less proximal to the prediction time point. Still, we wish to emphasize that our lowest auROC (.84) is still quite good, and the benefit of advanced notice likely outweighs the cost to performance.

The relative ordering of important features remained somewhat consistent across models. Past use, future efficacy, and craving were the top three features for all models. However the magnitude of their importance varied by lag time. For the 2-week lagged models, these top features were overall less important for the model's predictions compared to the no lag model. This is consistent with the 2-week lagged model's lower performance, compared to the no lag model.<!--what are the implications of noting this.  How did they vary and why important enough to mention here?-->. Additionally, for the two-week model future risky situations emerged as an important feature, whereas with the no lag model past stressful events were more important. <!--again, need to unpack this a bit more.  You can focus on how recent past experiences may be more improtant for immediate model whereas expectationns abou the future carry more weight when predicting further into the future.   But be careful to not over-interpret descritive differences in shaps.   We need to likely acknowledge the descriptive nature of our analysis of SHAPS.-->

<!--KW: I think we should actually remove this last sentence for fear of over-interpretting. It is not true that future foward-facing features were always more important for the 2 week model compared to recent past use.-->


## Model Fairness
<!--need to acknowledge that we didnt assess this in past work and that the field is just now starting to develop methods and committment to these issues--> 
...
Computational solutions to mitigate these issues in the current data may also exist. We could explore upsampling non-advantaged group representation in the data (e.g., using synthetic minority oversampling technique). We also could adjust the penalty weights so that prediction errors for non-advantaged groups are weighted more heavily than prediction errors for majority groups. We could also consider using personalized modeling approaches that consider the characteristics and behaviors important to an individual rather than generalizing across a population. For example, state space models inherently capture time series data and allow for the modeling of how an individual's risk evolves over time from observable and latent states. <!--this ideas about state space models need to be described more carefully.  Not clear to the reader why such models will be fairer unless you make it more clear that these models learn weights for each person.-->

The models also performed more poorly for women compared to men, despite the fact that they were well represented. This finding suggests representation in our data is not the only factor affecting model fairness. We chose our EMA items based on domain expertise and years of relapse risk research. It is possible that these constructs more precisely describe relapse risk factors for men than for women. <!--Why?  Need to acknowledge that past research on risk factors has often used predominatly male samples - perhaps due both to the base rates of AUD by sex but also ...  not sure how to make the point about biases in who we study.  But if you want to make this point, we need to be able to point to evidence this is true.  We need cites that show this or cites of people that have looked and confirmed these biases-->This could mean that more research is needed to identify relapse risk factors for women (and other groups underrepresented in the literature more broadly). 

Additionally, data driven (bottom-up) approaches to creating features could be one way to remove some of the bias in domain driven (top-down) approaches. For example, using natural language processing on text message content could allow for new categories of features to emerge.   <!--I dont think many people will understand this without a bit more elaboration.-->   

<!--there might also be a paragraph that suggests that marginnalized (or other descriptor) groups might also have lower trust that leads them to under-report lapses and/or risk factors.  This could be contriubuting to all the fairness problems (I dont think this is true now given tianruns models but its fine to speculate this).  We might need to compare lapse rates across these groups but then acknowledge that even with comparable lapse rates that doesnt confirm comparable reporting-->
