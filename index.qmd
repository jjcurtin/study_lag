...
<!-- Can we adjust or pivot to precision mental health quickly or even start there? Maybe make clear the extra importance of time when cconsidering chronic conditions that change over time?  Tradition precision medicence as mostly about how, not when.-->
...
<!-- shift to SUD in next paragraph is a bit abrupt and sensing and algorithms are not only good for SUD?  Maybe get to SUD first with project match?   Then acknowledge problems with match (distal and low dimensional) coudl be done in one paragraph I believe -->

<!-- Personal Sensing and Machine Learning -->

<!-- reverse order of sentence to make structure clearer-->
Recent advances in both personal sensing and machine learning may address both of these barriers to successful precision mental health.
<!-- I see your use of personalized interventions in this sentence too.  How do we want to use that vs. precision mental health vs. JITAI? -->
<!--Using algorithms to time (*when*) and select personalized interventions (*how*) for substance use may now be possible, thanks to personal sensing and high-dimensional machine learning algorithms. -->

Personal sensing allows for frequent, longitudinal monitoring of individuals' affect, behavior, and cognitions in their day-to-day lives [@mohrPersonalSensingUnderstanding2017]. These data can then be used as inputs for machine learning algorithms to predict the probability of specific outcomes (e.g., lapse risk) and identify the top features contributing to those predictions.
<!-- sensing measures changes in proximal risks with high temporal precision.   Machine learning allows for high dimensional inputs.  ML use EMA to produce temporally precision predictions about future risks.  Tools from AI can be applied to models to understand which factors are specific to that person at that moemnt in time.  -->
...
<!-- do we still need to justify lapse like you did in a previous verison? -->
<!-- lets use posterior estimates for auROC -->
<!-- too much detail RE raw and change features -->
