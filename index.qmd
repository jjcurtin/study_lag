...
Panel B in @fig-1 shows the difference in performance of each model by demographic group. All group comparisons for race, sex at birth, and income were significant (probability > .95) across models. On average there was a median increase in auROC of `r sprintf("%1.3f", pp_dem_contrast |> filter(contrast == "not white vs non-hispanic white") |> pull(median)) |> median()` (range `r sprintf("%1.3f", pp_dem_contrast |> filter(contrast == "non-hispanic white vs not white") |> pull(median)) |> min()`-`r sprintf("%1.3f", pp_dem_contrast |> filter(contrast == "not white vs non-hispanic white") |> pull(median)) |> max()`) for non-Hispanic White participants compared to participants who were not White. On average there was a median increase in auROC of `r sprintf("%1.3f", pp_dem_contrast |> filter(contrast == "female vs male") |> pull(median)) |> median()` (range `r sprintf("%1.3f", pp_dem_contrast |> filter(contrast == "female vs male") |> pull(median)) |> min()`-`r sprintf("%1.3f", pp_dem_contrast |> filter(contrast == "female vs male") |> pull(median)) |> max()`) for male participants compared to female participants. On average there was a median increase in auROC of `r sprintf("%1.3f", pp_dem_contrast |> filter(contrast == "below poverty vs above poverty") |> pull(median)) |> median()` (range `r sprintf("%1.3f", pp_dem_contrast |> filter(contrast == "below poverty vs above poverty") |> pull(median)) |> min()`-`r sprintf("%1.3f", pp_dem_contrast |> filter(contrast == "below poverty vs above poverty") |> pull(median)) |> max()`) for participants above the federal poverty line compared to participants below the federal poverty line. A table of model performance for each demographic group separately by model can be found in the supplement.  
...
Panel B shows the local feature importance scores colored by high or low feature value for the 336 hour lag model. <!--add local fi back to methods, comment on direction of features - ex. income. Also reference supplement with other local importance plots-->. 
