...
<!--At 4361 words not including 150 word abstract or discussion (about 500 words).-->
...
<!-- Not sure about role of MATCH in this paper.  You arent really doing treatment selection with these algorithms.  You are predicting when.  We argue we can use this for select but we dont really evaluate that in this paper -->
...
<!-- should we break apart the next paragraph into two ideas.  First that we can predict well for serveral different window widths and next that the narrow width immediate windows are good for JIT in a DTx.  Then you can transition to the neeed for more advanced warning for interventions and supports that are not completely digital - txt, support from a friend, etc.-->
...
<!-- we say we list all measures.  Do we need to indicate that we have many other trait and state measures that we dont use here but report on OSF? -->
...
<!-- I think we need a figure to show the prediction windows, feature scoring period (what did we call that in the last paper), and the lag  -->

<!-- Can we follow the text from the original EMA paper a bit more closely here regarding rolling windows and also the fence? -->
...
<!-- but we didnt use day/time features, correct?->
A total of 279 features were derived from two data sources: <!--286 if including time of day and day of week of start of prediction window-->
...
#### Model Evaluation
<!-- dont we need more detail about these models, priors, transform of auc?  Report in supplement or here? -->
...
<!-- didnt these comparisons come from the same bayesian models as the one reported above for model evaluation?  Maybe this shoudl be one section describing the bayseian model and how it is used to evaluate each model overall and make contrasts among models? -->
...
<!-- Should these analyses and their rationale have been in the intro?  SHould we have more detailed aims in the intro? -->
...
<!-- we need HDI for CI I believe.  We need to look into that using tidyposterior -->
...
### Baseline Contrasts
<!-- would these all be clearer if they focused on how the model performance decreased for lagged vs. baseline model? -->
...
<!--do we know the numbers in each of the two groups for each characteristic?  Cant get that from demos table.  Maybe report it here? Or as second paragraph in demos section?  -->

<!-- again, maybe focus on decrease in performance in protected group? -->
@fig-fairness shows the difference in performance of each model by demographic group. All group comparisons for race, sex at birth, and income were significant (probability > .95) across models. On average there was a median increase in auROC of `r sprintf("%1.3f", pp_dem_contrast |> filter(contrast == "non-hispanic white vs not white") |> pull(median)) |> median()` (range `r sprintf("%1.3f", pp_dem_contrast |> filter(contrast == "non-hispanic white vs not white") |> pull(median)) |> min()`-`r sprintf("%1.3f", pp_dem_contrast |> filter(contrast == "non-hispanic white vs not white") |> pull(median)) |> max()`) for non-Hispanic White participants compared to participants who were not White. On average there was a median increase in auROC of `r sprintf("%1.3f", pp_dem_contrast |> filter(contrast == "male vs female") |> pull(median)) |> median()` (range `r sprintf("%1.3f", pp_dem_contrast |> filter(contrast == "male vs female") |> pull(median)) |> min()`-`r sprintf("%1.3f", pp_dem_contrast |> filter(contrast == "male vs female") |> pull(median)) |> max()`) for male participants compared to female participants. On average there was a median increase in auROC of `r sprintf("%1.3f", pp_dem_contrast |> filter(contrast == "above poverty vs below poverty") |> pull(median)) |> median()` (range `r sprintf("%1.3f", pp_dem_contrast |> filter(contrast == "above poverty vs below poverty") |> pull(median)) |> min()`-`r sprintf("%1.3f", pp_dem_contrast |> filter(contrast == "above poverty vs below poverty") |> pull(median)) |> max()`) for participants above the federal poverty line compared to participants below the federal poverty line. A table of model performance for each demographic group separately by model can be found in the supplement.  
