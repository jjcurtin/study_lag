...
beta_probs <- function(x, model) {
  alpha <- model$alpha
  beta <- model$beta
  intercept <- model$intercept
  
  # Calibrate probabilities
  stats::pbeta(x, alpha, beta) ^ (1 / (1 + exp(-intercept)))
}

# Calibrate the probabilities
preds_prob$preds_beta <- beta_probs(preds_prob$.pred_yes, beta_model_old)
```


Testing above matches other method for beta calibration in split that worked
