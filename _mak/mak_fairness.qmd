...
feat_preds <- feat_preds |> 
  mutate(demo_sex = if_else(demo_sex == "Female", "female", "male"),
         ...
         demo_age = if_else(demo_age >= 55, "older", "younger"))
```
...
auroc_dem <- get_auroc_by_dem(df = feat_preds, var = "demo_sex") |> 
  ...
  left_join(get_auroc_by_dem(df = feat_preds, var = "demo_age"), by = "outer_split_num") 
```
...
```{r}
auroc_dem |> 
  write_csv(here::here(path_models, str_c("test_auroc_dem_", window, "_", lead, 
                                          "_", version, "_", cv, ".csv")))  
```
