---
title: "Make lapse labels for for 1 week window with `r params$lag` lag"
...
This script is based on the EMA study script for making the three windows with updates to apply to only 1week window and buffers that correspond to 24 hours + lag time in hours (lag hours = 24, 72, 168, 366). Notably, we have changed made the exclusion time for sampling no lapses around lapses less conservative (6 hours vs. 24 hours). There are other minor updates for qmd and current lab conventions.
...
- with all lapses we will not sample no_lapses +/- *6* hours from lapse   

- invalid lapses were also not sampled from for no_lapses within *6* hours of invalid lapse  
...
Make labels with 1 week window duration and `r params$lag` lag.    
Labels will start at midnight on start day + 24 hours + lag hours
```{r future_map}   
if (lag == 0) {
  labels <- subids |> 
    future_map_dfr(\(subid) get_lapse_labels(subid, lapses, dates, 
                                    buffer_start = 60 * 60 * 24, window_dur = 604800)) |>  # window_dur is 1 week
    glimpse()
} else {
  labels <- subids |> 
    future_map_dfr(\(subid) get_lapse_labels(subid, lapses, dates, 
                                    buffer_start = 60 * 60 * (24 + lag), window_dur = 604800)) |>  # window_dur is 1 week
    glimpse()
}
```
...
```{r}
labels |> 
  write_csv(here::here(str_c(path_lag, "/labels_1week_", lag, "lag.csv"))) 
```
