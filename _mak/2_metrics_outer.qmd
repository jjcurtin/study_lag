...
if(!file.exists(here::here(path_models, str_c("outer_preds_", 
  ...
  # manually update for now if running dem 
  dem <- FALSE
  
  if (dem == TRUE) {
    # update outer resampling splits and model to dem
...
This calculates auROC using test sets (out loop held out folds). Uses probability corrected (prob_logi) probabilities.

```{r}
if (model == "main") {
 metrics_test <- probs_out |> 
  nest(.by = outer_split_num, .key = "preds") |> 
  mutate(auroc = map(preds, \(preds) roc_auc(preds, prob_logi, 
                                             truth = label))) |> 
  select(-preds) |> 
  unnest(auroc) |> 
