...
params:
  ...
  cv: "nested"
  model: "dem_main"
  dem: TRUE #set true to use 6 repeats of 5 fold cv for outer fits
editor_options: 
...
# NOTE: training controls overwrites path_batch but it matches   
  ...
  if (dem == TRUE) {
    ...
    model <- "dem_stratify"
    lapse_strat <- read_csv(file.path(path_batch, "input/lapse_strat.csv"))
  }
...
if(!file.exists(here::here(path_models, str_c("outer_preds_", 
  ...
  d <- format_data(d, lapse_strat = NULL) |> 
    arrange(label_num) |> 
