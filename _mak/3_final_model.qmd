---
title: "Fits and evaluates final model selected from inner resampling method and fit on all data"
author: "Kendra Wyant"
...
### Notes
This script fits and evaluates final model selected from inner resampling method and fit on all data. It returns raw and calibrated probabilities and shapley values.
...
```{r}
fit_predict_eval <- function(d, config_best){
  ...
  model_best <- fit_best_model(config_best, feat = d_in, "classification")
  
  d_out <- rec_prepped |> 
    bake(new_data = d)   # no id_obs because not included in d_in
    
  preds_prob <- predict(model_best, d_out,
                        type = "prob")

  # train calibration model on all data
  # set.seed(2468)
  # cal_split <- d_in |> 
  ...
  # iso calibration
  iso <- predict(model_best, d_out,
                 type = "prob") |> 
    mutate(truth = d_out$y) |> 
  ...
  beta <- predict(model_best, d_out,
   ...
   probs_out <- tibble(id_obs = d$id_obs,
 ...
 # SHAP in held out fold
  shaps_out <- SHAPforxgboost::shap.prep(xgb_model = extract_fit_engine(model_best),
                     X_train = d_out |> select(-y) |>  as.matrix()) |>
    # add id_obs by multiple of number of features
    mutate(id_obs = rep(d_out$id_obs, times = ncol(d_out) - 1)) |>
    relocate(id_obs)
...
if(!file.exists(here::here(path_models, str_c("final_preds_", cv_name,
                                           "_", window, "_", lead, "_", 
 ...
 d <- format_data(d) |> 
  ...
  all <- fit_predict_eval(d, config_best)
  
  
  probs_out <- all |> 
    pluck("probs_out") |> 
    write_rds(here::here(path_models, str_c("final_preds_", cv_name,
                                           "_", window, "_", lead, "_", 
                                           version, "_", cv, "_", model,
                                           ".rds")))
  shaps_out <- all |> 
      ...
      write_rds(here::here(path_models, str_c("final_shaps_", 
                                             cv_name, "_",
  ...
  probs_out <- read_rds(here::here(path_models, str_c("final_preds_", cv_name,
                                           "_", window, "_", lead, "_", 
                                           version, "_", cv, "_", model,
                                           ".rds")))
  
  shaps_out <- read_rds(here::here(path_models, str_c("final_shaps_", 
                                                     cv_name, "_",
                                                     window, "_", lead, "_", 
                                                     version, "_", 
                                                     cv, "_", model, ".rds")))
...
Group Shaps
```{r}
if(!file.exists(here::here(path_models, str_c("final_shapsgrp_", 
                                              cv_name,
                                              "_", window, "_", lead, "_",
                                              version, 
                                              "_", cv, "_",  model, ".rds")))){
  ...
  shaps_out_grp |>
    write_rds(here::here(path_models, str_c("final_shapsgrp_", 
                                            cv_name, "_",
